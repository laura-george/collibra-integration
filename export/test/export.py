...
type_ids = ["BOOLEAN", "TINYINT", "SMALLINT", "INT", "BIGINT", "FLOAT", "DOUBLE", "STRING",
    "VARCHAR", "CHAR", "BINARY", "TIMESTAMP_NANOS", "DECIMAL", "DATE", "RECORD", "ARRAY", "MAP"]
    ...
    with conn:
        ...
        if asset_name:
            if asset_type == "Database":
                ...
                tables = conn.list_datasets(db=db_name, name=asset_name.split(".")[1])
                t = tables[0]
    ...
    def define_tags(tag):
        ...
        with conn:
            ...
            if action == "assign":
                ...
                for custom_attr in configs['mapped_collibra_attributes']:
                    if nmspc_key[0] == custom_attr['okera_namespace']:
                        if type == "Column":
                            tab_col = name.split(".")
                            try:
